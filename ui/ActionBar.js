class ActionBar {
  constructor() {
    ...
    this.subMenuGen = []; //not in use, remove
  ...
  openActionMenu() {
    ...
    ElUtils.fadeIn(this.minOpacity, this.maxOpacity, this.fadeDuration, box, 0.1);
    ElUtils.fadeIn(
      this.minOpacity,
      this.maxOpacity,
      this.fadeDuration,
  ...
  closeActionMenu() {
    ...
    const box = document.querySelector(".action_bar_submenu_list");
    ElUtils.fadeOut(this.minOpacity, this.maxOpacity, this.fadeDuration, box, 0.1);
    ElUtils.fadeOut(
      this.minOpacity,
      this.maxOpacity,
      this.fadeDuration,
  ...
  popularizeSlotBar(init = false) {
    this.actionBarItems.forEach((item, i) => {
      ...
      //check cld, is on cooldown, add visual cooldown graphic
      if (this.cooldownItems[item.menu][item.id]) this.addCooldownIcon(this.cooldownItems[item.menu][item.id], item);
    });
    this.selectActionbarItem();
    if (init) {
  ...
  //new cld logic
  addCooldownIcon = (item, {itemType, itemID}) => {
    const img = document.createElement("img");
    img.classList.add(
      "cooldown_icon",
      this.buildCooldownClassString(itemType, itemID)
    );

    item.appendChild(img);
  }
  updateCooldownIcon = ({itemType, itemID}, percPassed) => {
    const img = document.querySelectorAll( //select both from actionbar and submenu
      `.${this.buildCooldownClassString(itemType, itemID)}`
    );
    img.forEach(
      (i) =>
        (i.src = `./spacemap/ui/actionBar/cooldown/${percPassed}.png`)
    );
  }
  removeCooldownIcon(items, {itemType, itemID}) {
    delete this.cooldownItems[itemType][itemID];
    
    items.forEach((item) => {
      item.querySelector(".cooldown_icon").remove();
    });
  }
  buildCooldownClassString(itemType, itemID) {
    return `cooldown_${itemType}_${itemID}`;
  }
  cooldownHandler(cldItem) {
    const {timeSpent, timeEnd} = cldItem;

    const percComplete = Math.round((timeSpent / timeEnd) * 100);

    const items = document.querySelectorAll(
      `[item_info="${cldItem.itemType}_${cldItem.itemID}"]`
    );

    if (percComplete >= 100) return this.removeCooldownIcon(items, cldItem);

    items.forEach((item) => {
      if (cldItem.timeSpent == 0) return this.addCooldownIcon(item, cldItem);
      this.updateCooldownIcon(cldItem, percComplete);
    });
  ...
  selectItemsToCooldown(data) {
    data = trimData(data);
    const itemType = this.getItemPacket(data[0], true);
    const itemID = data[1];
    const time = data[2];
    console.log(this.actionBarItems);
    if (itemID === "a") this.getAllItemsOfType(itemType).forEach((i) => this.setItemCooldown(i.itemID, itemType, time));
    else this.setItemCooldown(itemID, itemType, time);
  }

  setItemCooldown(itemID, itemType, time) {
    if (this.cooldownItems[itemType][itemID]) {
  ...
  getItemPacket(key, reverse = false) {
    ...
    if (reverse) return menuPackets.indexOf(key);
    return menuPackets[key];
  }
  //new items logic
  getAllItemsOfType(type) {
    return SUB_MENU_ITEMS[type];
  }
}
